                ...
                <div class="mt-2 bg-white shadow overflow-hidden rounded-lg">
                    ...
                    <div class="border-t border-gray-200 flex justify-between items-center">
                        <div class="flex-grow px-4 py-2">
                            <input type="text" v-model="searchQuery" class="w-full border rounded-md py-2 px-3"
                                placeholder="Search..." @input="handleSearch">
                        </div>
                        <button @click="searchProfileData"
                            class="bg-blue-500 text-white py-2 px-4 mr-2 rounded-md">Search</button>
                    </div>
                    <!-- Filtered Results -->
                    <div v-if="filteredResults.length === 0" class="px-4 py-2 text-gray-500"></div>
                    <div v-else>
                        <div v-for="result in filteredResults" :key="result.id"
                            class="px-6 py-2 border-b bg-slate-500 mx-4 m-2 rounded-md" @click="populateSearch(result)">
                    ...
                    <div class="border-t border-gray-200">
                        <dl>
                            ...
                            <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900 sm:col-span-2">{{ userData.place_of_birth }}</dd>
                            </div>
                            <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                    <dt class="text-sm font-medium text-gray-500">Date</dt>
                                    <dd class="mt-1 text-sm text-gray-900 sm:col-span-2">{{ userData.date_of_birth }}</dd>
                            </div>
                            <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            ...
                            <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                        ...
                                        <dd class="mt-1 text-sm text-gray-900 sm:col-span-2">{{ userData.department }}</dd>
                            </div>
                        </dl>
                    </div>
                </div>
